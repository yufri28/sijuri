...
<body>
    ...
    <section class="home-section">
        <div class="text">Data Perhitungan Nilai Peserta Lomba
            <div class="periode-perhitungan">
                <input type="text" class="form-control" id="perhitungan-aktif" name="periode-aktif" readonly>
            </div>
                        ...
                        <table id="tabel-normalisasi-bobot" class="table table-striped" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: left;"><?= $subkriteria_item['subkriteria_keterangan'] ?>
                                    </td>
                        ...
                        <table id="tabel-nilai-kriteria" class="table table-striped" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th style="text-align: center;">Materi Penilaian</th>
                                    <?php
                                                $nomor = 1;
                                                ?>
                                    <?php foreach ($kriteria as $kriteria_item) : ?>
                                    <th style="text-align: center;" title="Lagu ke-<?= $nomor++ ?>">
                                        <?= $kriteria_item['nama_kriteria'] ?></th>
                                    <?php endforeach; ?>
                                </tr>
                            </thead>
                            <tbody>
                                        ...
                                        <?php
                                                        ...
                                                        foreach ($kriteria as $kriteria_item) {
                                                            ...
                                                            $sql_nilai_subkriteria = "SELECT subkriteria_keterangan 
                                                                                        ...
                                                                                        WHERE penilaian.id_kriteria = $id_kriteria AND penilaian.id_alternatif = $id_alternatif AND periode.status_periode = 'Aktif'";
                        ...
                        <table id="tabel-nilai-kriteria" class="table table-striped" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th style="text-align: center;" title="Lagu ke-<?= $nomor ?>">
                                        <?= $kriteria_item['nama_kriteria'] ?></th>
                                    <th style="text-align: center;" title="Total persentase lagu ke-<?= $nomor ?>">Total
                                        Persentase</th>
                                    <?php $nomor++; ?>
                                    <?php endforeach; ?>
                                </tr>
                                        ...
                                        <?php
                                                    ...
                                                    foreach ($kriteria as $kriteria_item) {
                                                        ...
                                                        $sql_nilai_subkriteria = "SELECT subkriteria_keterangan 
                                                                                    ...
                                                                                    AND periode.status_periode = 'Aktif'";
                                                        $result_nilai_subkriteria = $koneksi->query($sql_nilai_subkriteria);
                                    ...
                                    <td style="text-align: center;">
                                        <?php
                                                    foreach ($kriteria as $kriteria_item) {
                                                        ...
                                                        // Query untuk mendapatkan nilai dan subkriteria_id dari tabel penilaian berdasarkan kriteria, alternatif, user, dan periode aktif
                                                        $sql_nilai = "SELECT nilai, subkriteria_id 
                                                                        FROM penilaian 
                                                                        WHERE id_kriteria = $id_kriteria 
                                                                        AND id_alternatif = $id_alternatif 
                                                    ...
                                                    if ($result_simpan) {
                                                        echo number_format($nilai_akhir, 2); // Tampilkan nilai akhir dengan format dua desimal
                                                    } else {
    ...
    $(document).ready(function() {
        $('#tabel-normalisasi-bobot, #tabel-nilai-kriteria, #tabel-nilai-utility, #tabel-nilai-akhir')
            .DataTable();
    });
    </script>
    <script>
    // Fungsi untuk mengisi nilai input periode aktif
    function isiNilaiPeriodeAktif() {
        <?php
                // Koneksi ke database
                $koneksi = connectDB();

                // Query untuk mengambil tahun dan bulan periode yang aktif
                $sql_periode_aktif = "SELECT tahun_periode, bulan_periode FROM periode WHERE status_periode = 'Aktif'";
                $result_periode_aktif = $koneksi->query($sql_periode_aktif);

                // Periksa apakah query berhasil dan hasilnya tidak kosong
                if ($result_periode_aktif && $result_periode_aktif->num_rows > 0) {
                    $row_periode_aktif = $result_periode_aktif->fetch_assoc();
                    $tahun_periode = $row_periode_aktif['tahun_periode'];
                    $bulan_periode = $row_periode_aktif['bulan_periode'];
                    echo "var periodeAktif = 'Periode: $tahun_periode $bulan_periode';";
                } else {
                    echo "var periodeAktif = 'Tidak ada periode aktif';";
                }
                ?>

        // Isi nilai input periode aktif dengan informasi yang diperoleh dari database
        document.getElementById('perhitungan-aktif').value = periodeAktif;
    }

    // Panggil fungsi saat halaman dimuat
    window.onload = isiNilaiPeriodeAktif;
    </script>
</body>
